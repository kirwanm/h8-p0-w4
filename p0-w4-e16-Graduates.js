...
function graduates(students) {
  ...
  for(var i=0; i<students.length; i++) {
    var duplikat = false;
    for(var j=0; j<classG.length; j++) {
      if(classG[j]===students[i]['class']) {
        duplikat = true;
      }
    }
    if(duplikat===false) {
      classG.push(students[i]['class']);
    }
  }
  //console.log(classG)
  for(var i=0; i<classG.length; i++) {
    for(var j=0; j<students.length; j++) {
      if(students[j]['class']===classG[i] && students[j]['score']>75) {
        var objLulus = {};
        objLulus['name'] = students[j]['name'];
        objLulus['score'] = students[j]['score'];
        arrHasil.push(objLulus);
      }
    }
    objClass[classG[i]] = arrHasil
    arrHasil = [];
  }
  return objClass;
}
